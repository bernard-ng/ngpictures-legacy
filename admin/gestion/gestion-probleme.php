...
if(isset($_SESSION['id']) and !empty($_SESSION['id'])){
	...
	$bleme = $db->query("SELECT * from problemes order by id desc");
	$Nb = $bleme->rowcount();

	if(isset($_GET['q']) and !empty($_GET['q'])){

		$q = htmlspecialchars($_GET['q']);
		$bleme = $db->query("SELECT * from problemes where concat(userID,probleme,date_pub,statut) like '%".$q."%' order by id desc ");
		$Nb = $bleme->rowcount();
	}
	...
	<div class="row">
		<div class="ng-panel panel-default panel ng-panel-active">
			...
			<li class="list-group-item">
				<div class="media">
					<div class="media-left media-top">
						<a href="pages/membres/profil.php?id=<?= $b['userID']?>">
							<img class="media-object img img-circle" src="pages/membres/Avatar/40-40/<?= getUserProfil($b['userID'])?>" width="40" height="40">
						</a>
					</div>
					<div class="media-body">
						<a href="pages/membres/profil.php?id=<?= $b['userID']?>">
						<h5 class="media-heading"><?= getUserPseudo($b['userID'])?></h5></a>
						<h6><?= nl2br(user_mention_verif($b ['probleme'])) ?></h6>
						<span class="pull-right">
						<time><span class="glyphicon glyphicon-time"></span> <?= getRelativeTime($b['date_pub'])?></time></span>
					</div>
				</div>
