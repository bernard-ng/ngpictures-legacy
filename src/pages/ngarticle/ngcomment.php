...
if(isset($_SESSION['id']) and !empty($_SESSION['id'])) {
    if(isset($_GET['id']) and !empty($_GET['id'])) {
        ...
        if($news->rowcount() ==1) {
            ...
            $articleID= $news['id'];
            $poster= getUserPseudo($news['posterID']);
            ...
            $commentaire= $db->prepare("SELECT * FROM ngcommentaire WHERE articleID=? order by date_pub desc ");
            $commentaire->execute(array($articleID));

            $comN = $db->prepare("SELECT * FROM ngcommentaire WHERE articleID=? order by date_pub desc ");
...
<head>
    ...
    <title>Commentaire | <?php echo $titre ?></title>
...
<div class="col-xs-12 col-sm-8 col-md-8 col-lg=8" >
    <div class="row">
        ...
        <div class="ng-panel panel-heading ng-margin-default">COMMENTAIRE<?php echo $t = (getBlogInfo($articleID, "commentaire") > 1)? "S" : ""  ?> <span class="ng-badge badge"><?php echo $com ?></span></div>
