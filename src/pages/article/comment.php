...
if(isset($_SESSION['id']) and !empty($_SESSION['id'])) {
    if(isset($_GET['id']) and !empty($_GET['id'])) {
        ...
        if($news->rowcount() ==1) {
            ...
            $commentaire= $db->prepare("SELECT * from commentaire where articleID=? order by date_pub desc ");
            $commentaire->execute(array($articleID));

            $comN = $db->prepare("SELECT * from commentaire where articleID=? order by date_pub desc ");
...
<div class="row">
    <div class="ng-panel panel-primary panel ng-panel-active">
        <div class="ng-panel panel-heading ng-margin-default">COMMENTAIRE<?php echo $t = (getArticleInfo($articleID, "commentaire") > 1)? "S" : ""  ?> <span class="ng-badge badge"><?php echo $com ?></span></div>
