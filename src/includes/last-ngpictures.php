...
<div class="ng-alert ng-panel panel-primary">
    ...
    <div class="panel-body ">
            Salut <?= getUserPseudo($_SESSION['id']) ?> as-tu dejà lu mon dernier article ?
        <br><br>
        <article class= "card">
            <header class="card--thumb">
                <a href="\ngarticle\ngarticle.php?id=<?= $newss['id']?>"><img src="\ngarticle\miniature\640-640\<?= getBlogThumb($newss['id'])?>" /></a>
            </header>
            <div class="card--date">
                <span class="card--date--day"><?= date('d');?></span>
                <span class="card--date--month"><?= date('M');?></span>
            </div>
            <div class="card--body">
                <div class="card--category">actu</div>
                <div class="card--subtitle"><a href="\ngarticle\ngarticle.php?id=<?= $newss['id']?>"><?= truncate($newss['titre'],20)?></a></div>

                <p class="card--desc">
                    <?= user_mention_verif(htag(truncate($newss['contenu'],90)))?>
                    <br>
                    <button class="btn btn-primary btn-xs ng-btn"><a href="\ngarticle\ngarticle.php?id=<?= $newss['id']?>">
                    <span class="glyphicon glyphicon-chevron-right"></span></a>
                    </button>
                </p>
            </div>
            <footer class="card--footer">
                <span class="glyphicon glyphicon-time"></span> <?= getRelativeTime($newss['date_pub']);?>
            </footer>
        <pages/article>
    </div>
</div>
